...
class MaskedAutoencoderViT(nn.Module):
    ...
    def grid_size(self):
        return self.patch_embed.patches_resolution
    ...
    def interpolate_pos_encoding(self, x, w, h, embed):
        ph, pw = self.patch_embed.patch_size
        npatch = x.shape[1] - 1
        N = embed.shape[1] - 1
        if npatch == N and w == h:
            return embed
        class_pos_embed = embed[:, 0]
        patch_pos_embed = embed[:, 1:]
        dim = x.shape[-1]
        w0 = w // ph
        h0 = h // pw
    ...
    def unpatchify(self, x, original_imgs):
        """
        x: (N, L, patch_size[0]*patch_size[0]*in_chans)
        ...
        """
        ph, pw = self.patch_embed.patch_size
        h = original_imgs.shape[2] // ph
        w = original_imgs.shape[3] // pw
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        x = x + self.interpolate_pos_encoding(x, w, h, self.pos_embed)[:, 1:, :]
    ...
    def forward_decoder(self, x, ids_restore, original_imgs):
        _, _, w, h = original_imgs.shape
...
def mae_vit_base_patch16_1ch_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
mae_vit_base_patch16_1ch = mae_vit_base_patch16_1ch_dec512d8b  # decoder: 512 dim, 8 blocks
